...
export default async function (entries: VTEXYCore.EntryArgs) {
  ...
  bs.emitter.on('init', (bsInstance: any) => {
    bsInstance.options.get('urls').forEach((item: any) => {
      const link: string = terminalLink(item[1], item[1], {
        fallback: () => item[1],
      })
      if (item[0] === 'external') {
    ...
    //     VTEXYCore.options.noSSR ? chalk.red('disabled') : chalk.green('enabled')
    //   }`
    // )
  })
