...
const parseCollection = async ({ content, object, cookie }) => {
  const shelv = VTEXYCore.runtime.dataTree.shelves.find(
    x => x.path.indexOf(object.layout + '.jsonc') > 0
  );

  const template = VTEXYCore.runtime.contentTree.shelves.find(
    x => x.name == object.layout + '.vm'
  ).content;

  try {
    let { data: products } = await api.get(
      `https://${VTEXYCore.options.account}.vtexcommercestable.com.br/api/catalog_system/pub/products/search?fq=productClusterIds:${content.productCluster}`,
      {
        headers: {
          Cookie: cookie
        }
...
const postHTMLVtex = ({ layout, cookie }) => (tree: any) => {
  return new Promise(async resolve => {
    // List placeholders id in html template
    let placeholders = [];
    tree.match({ tag: 'vtex:contentPlaceholder' }, (node: any) => {
    ...
    await Promise.all(
      placeholders.map(async id => {
        ...
        let html : any = await Promise.all(
          placeholder.map(
            async object =>
              await Promise.all(
        ...
        tree.match({ tag: 'vtex:contentPlaceholder', attrs: { id } }, (node: any) => {
          if (placeholder) {
...
export default async args => {
  ...
  if (args) {
    let htmlFile: string | undefined | Buffer = VTEXYCore.runtime.contentTree.templates.find(
