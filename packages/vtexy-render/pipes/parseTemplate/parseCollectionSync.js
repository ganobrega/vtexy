...
module.exports = async ({ content, object, cookie }) => {
  let shelv = JSONC.parse(
    await fs.readFileSync(path.join(process.env.VTEXY_DATA, 'shelves', object.layout + '.jsonc'), 'utf8')
  );

  let template = await fs.readFileSync(path.join(process.env.VTEXY_CONTENT, 'shelves', object.layout + '.vm'), 'utf8');

  try {
    let { data: products } = await api.get(
      `https://${process.env.VTEX_ACCOUNT}.vtexcommercestable.com.br/api/catalog_system/pub/products/search?fq=productClusterIds:${content.productCluster}`,
      {
        headers: {
          Cookie: cookie
        }
    ...
    let shelfs = products.map(product => Velocity.render(template, { product }, {}));
