...
async function loadDataTree() {
    ...
    function defineWebsiteUserAgentType(item: VTEXYCore.DirTree, key: any, parentValue: any, context: any) {
      ...
      let primary = mapValuesDeep(
        context.obj,
        (x: any) => x.type === 'website' && x.parent == undefined,
        deepOpts
      );
...
async function loadContentTree() {
    ...
    const contentTree = {
      ...
      templates: tree .find(x => x.name == 'templates' && x.type == 'directory') ?.children.filter(x => x.name != 'subtemplates' && x.type != 'directory'),
      subTemplates: tree .find(x => x.name == 'templates' && x.type == 'directory') ?.children.find(x => x.name == 'subtemplates' && x.type == 'directory')?.children,
